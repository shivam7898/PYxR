...
aa = pd.DataFrame({'ID': (1, 2, 3), 'A': ('a1', 'a2', 'a3')}) 
bb = pd.DataFrame({'ID': (1, 2, 4), 'B': ('b1', 'b2', 'b4')}) 

ab_inner = aa.merge(bb, on = 'ID', how = 'inner')           #Inner Join
ab_left  = aa.merge(bb, on = 'ID', how = 'left')            #Left Outer Join
ab_right = aa.merge(bb, on = 'ID', how = 'right')           #Right Outer Join
ab_full  = aa.merge(bb, on = 'ID', how = 'outer')           #Full Outer Join
ab_cross = aa.merge(bb, how = 'cross')                      #Cross Join
...
ab_full <- py$ab_full |> `attr<-`('pandas.index', NULL) |> 
    mutate(across(where(is.list), q_NULL_to_NA)) |> as_tibble()
ab_cross <- py$ab_cross |> `attr<-`('pandas.index', NULL) |> 
    mutate(across(where(is.list), q_NULL_to_NA)) |> as_tibble()
